...
})(exports.TermType || (exports.TermType = {}));
...
})(exports.QuadPosition || (exports.QuadPosition = {}));
...
function equals(a, b) {
  ...
  if (PlainFactory.isQuad(a) || PlainFactory.isQuad(b)) {
    ...
    return equals(a.subject, b.subject) && equals(a.predicate, b.predicate) && equals(a.object, b.object) && equals(a.graph, b.graph);
  }

  switch (a.termType) {
    case 'Literal':
      {
        return a.termType === b.termType && a.value === b.value && b.datatype === b.datatype && b.language === b.language;
      }
...
class PlainFactory {
  constructor(opts = {}) {
    ...
    this.supports = opts.supports || {
      [exports.Feature.collections]: false,
      [exports.Feature.defaultGraphType]: false,
      [exports.Feature.equalsMethod]: false,
      [exports.Feature.id]: true,
      [exports.Feature.idStamp]: false,
      [exports.Feature.identity]: false,
      [exports.Feature.reversibleId]: true,
      [exports.Feature.variableType]: false
    };
  }
  /**
  ...
  static isQuad(obj) {
    ...
    return !!("subject" in obj && "predicate" in obj && "object" in obj);
  }

  namedNode(value) {
    const term = Object.create(RDFBase);
    term.termType = exports.TermType.NamedNode;
    term.value = value;
    return term;
  }
  ...
  blankNode(value) {
    const term = Object.create(RDFBase);
    term.termType = exports.TermType.BlankNode;
  ...
  literal(value, languageOrDatatype) {
    ...
    const term = Object.create(RDFBase);
    term.termType = exports.TermType.Literal; // Perf: Always set all properties to prevent the creation of unnecessary HiddenClass objects.

    term.language = typeof languageOrDatatype === "string" ? languageOrDatatype : undefined;
    term.datatype = typeof languageOrDatatype === "string" ? this.namedNode(datatypes.langString) : languageOrDatatype || this.namedNode(datatypes.string);
    term.value = value;
    return term;
  }
  ...
  quadruple(subject, predicate, object, graph) {
    return [subject, predicate, object, graph || this.defaultGraph()];
  }
  ...
  qdrFromQuad(original) {
    return [original.subject, original.predicate, original.object, original.graph || this.defaultGraph()];
  }

  qdrFromQdr(original) {
    return [original[0], original[1], original[2], original[3] || this.defaultGraph()];
  }
  /**
  ...
  termToNQ(term) {
    switch (term.termType) {
      case exports.TermType.BlankNode:
        return `_:${term.value}`;

      case exports.TermType.NamedNode:
        return `<${term.value}>`;

      case exports.TermType.Literal:
  ...
  quadrupleToNQ(quad) {
    const graph = !quad[exports.QuadPosition.graph] || quad[exports.QuadPosition.graph] === this.defaultGraph() ? '' : this.toNQ(quad[exports.QuadPosition.graph]) + ' ';
    return `${this.toNQ(quad[exports.QuadPosition.subject])} ${this.toNQ(quad[exports.QuadPosition.predicate])} ${this.toNQ(quad[exports.QuadPosition.object])} ${graph}.\n`;
  }

  quadToNQ(quad) {
    return this.quadrupleToNQ([quad.subject, quad.predicate, quad.object, quad.graph || this.defaultGraph()]);
  }
...
function isNamedNode(obj) {
  return isTerm(obj) && obj.termType === exports.TermType.NamedNode;
}
function isBlankNode(obj) {
  return isTerm(obj) && obj.termType === exports.TermType.BlankNode;
}
function isLiteral(obj) {
  return isTerm(obj) && obj.termType === exports.TermType.Literal;
}
const TermTypes = [exports.TermType.NamedNode.toString(), exports.TermType.BlankNode.toString(), exports.TermType.Literal.toString()];
function isSomeTerm(obj) {
  return isTerm(obj) && TermTypes.includes(obj.termType) === true;
}
const NodeTypes = [exports.TermType.NamedNode.toString(), exports.TermType.BlankNode.toString()];
function isNode(obj) {
...
function changeFactory(rdfFactory, factory, override) {
  if (shouldOverride(rdfFactory, override)) {
    ...
    exports.globalFactory = factory;
  } else if (typeof exports.globalFactory === "undefined" || override) {
    globalThis[rdfFactory] = factory;
    exports.globalFactory = factory;
  }
}
...
if (typeof Symbol !== "undefined") {
  ...
  exports.setup = function setup(factory = DefaultFactory, override = true) {
    changeFactory(rdfFactory, factory, override);
    exports.globalFactory = globalThis[rdfFactory];
  };

  exports.globalSymbol = rdfFactory;
} else {
...
exports.setup(DefaultFactory);
const createNS = ns => term => exports.globalFactory.namedNode(`${ns}${term}`);
let proxy;

if (typeof Proxy !== "undefined") {
  proxy = new Proxy(exports.globalFactory || {}, {
    ownKeys() {
      return exports.globalFactory && Object.keys(exports.globalFactory) || [];
    },

    getOwnPropertyDescriptor(_, k) {
      return Object.getOwnPropertyDescriptor(exports.globalFactory, k);
    },

    set(_, property, value) {
      exports.globalFactory[property] = value;
      return true;
    },

    get(_, property) {
      return exports.globalFactory[property];
    }

  });
} else {
  proxy = ['namedNode', 'blankNode', 'literal', 'defaultGraph', 'quad', 'quadruple', 'fromTerm', 'fromQuad', 'fromQdr', 'qdrFromQuad', 'qdrFromQdr', 'equals', 'id', 'toNQ', 'fromId', 'termToNQ', 'termFromNQ', 'quadrupleToNQ', 'quadToNQ', 'supports'].reduce((acc, key) => {
    acc[key] = (...args) => exports.globalFactory[key](...args);
...
exports.PlainFactory = PlainFactory;
exports.createNS = createNS;
exports.default = proxy$1;
exports.doc = doc;
